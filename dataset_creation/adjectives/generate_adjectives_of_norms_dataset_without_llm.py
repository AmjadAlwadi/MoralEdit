...
very_good_words = load_dataset(f"{datasets_path}/judgements/", data_files="very_good_dataset.json", split='train')
good_words = load_dataset(f"{datasets_path}/judgements/", data_files="good_dataset.json", split='train')
bad_words = load_dataset(f"{datasets_path}/judgements/", data_files="bad_dataset.json", split='train')
very_bad_words = load_dataset(f"{datasets_path}/judgements/", data_files="very_bad_dataset.json", split='train')
ok_words = load_dataset(f"{datasets_path}/judgements/", data_files="ok_dataset.json", split='train')

# Datasets for judgements to check later if no match was found in the first check
big_list_bad_words = load_dataset(f"{datasets_path}/judgements/", data_files="bad_and_very_bad_dataset.json", split='train')
big_list_good_words = load_dataset(f"{datasets_path}/judgements/", data_files="good_and_very_good_dataset.json", split='train')
...
def find_adjective(batch, indices):
    ...
    for i,idx in enumerate(indices):
        ...
        for word in batch["anti_norm"][i].split():
            ...
            if batch['action_moral_judgment'][i] > 0:
...
subejcts_dataset.to_json(f"{datasets_path}/norms/norms_adjectives.json")
